...
const Profile = () => {
  ...
  const checkIfFollowing = async (currentUserId, targetUserId) => {
    try {
      const res = await axios.get(
        `http://localhost:5000/subscriptions?followerId=${currentUserId}&followingId=${targetUserId}`
      );
  ...
  const handleUnsubscribe = async () => {
    try {
      const res = await axios.get(
        `http://localhost:5000/subscriptions?followerId=${user.id}&followingId=${profileUser.id}`
      );
  ...
  const fetchSubscriptions = async (userId) => {
    try {
      ...
      const [followingUsersRes, followerUsersRes] = await Promise.all([
        followingIds.length
          ? axios.get(
              `http://localhost:5000/users?${followingIds
                .map((id) => `id=${id}`)
                .join("&")}`
            )
        ...
        followerIds.length
          ? axios.get(
              `http://localhost:5000/users?${followerIds
                .map((id) => `id=${id}`)
                .join("&")}`
            )
          : Promise.resolve({ data: [] }),
  ...
  const petCountText = getWordForm(pets.length, [
    "питомец",
    "питомца",
    "питомцев",
  ]);
  const followerCountText = getWordForm(followers.length, [
    "подписчик",
    "подписчика",
    "подписчиков",
  ]);
  const followingCountText = getWordForm(following.length, [
    "подписка",
    "подписки",
    "подписок",
  ]);
  ...
  const handleMessageClick = async () => {
    try {
      const res = await axios.get(
        `http://localhost:5000/chats?participants_like=${user.id}`
      );
  ...
  const renderSkeleton = () => (
    <div style={{ maxWidth: 750 }}>
      ...
      <div style={{ display: "flex", alignItems: "center", marginBottom: 16 }}>
        <Skeleton circle width={100} height={100} style={{ marginLeft: 30 }} />
      ...
      {/* Buttons */}
      <div style={{ display: "flex", gap: 20, marginBottom: 24, marginLeft: 10, marginTop: 40 }}>
            ...
            {isOwnProfile ? (
              ...
              <>
                ...
                <button
                  className={styles.editButton}
                  onClick={
                    isFollowingProfileUser ? handleUnsubscribe : handleSubscribe
                  }
                >
